...
data.frame(Deletion = c("Listwise deletion", "Pairwise deletion", 
                        ...
                        "Growth Curve Models", " "),
           Single = c("Mean Substitution", "Regression Imputation", 
                      "Hot-Deck Imputation", "Averaging Available Items",
                      "Last Observation Carried Forward")) %>% 
  kable(., format = "html", 
        col.names = c("Data Deletion Methods", 
...
Listwise deletion means you drop every case that has missing data on any 
variable in the analysis. That's perhaps the most commonly used method because 
it is the default behavior for many statistical software packages. *It can only* 
*give you unbiased parameter estimates if the data are MCAR*, but even then you 
still lose power and precision due to shrinking sample size. But MCAR is a rare 
situation. You're far more likley to have MAR, and then using listwise deletion 
will give you biased results in addition to low power. If there are high rates 
missingness and many different patterns of missingness, you may not end up with
enough observations to do meaningful analyses.
...
This makes really strong assumptions about the missing items being 
interchangeable and about the factor structure. It may decrease variance the 
variance between items and will often decrease reliability. 

Individual growth curve models use the available longitudinal observations for 
each person to estimate longitudinal trajectories in a way that is similar to 
how available items are used to measure constructs. This can be misleading when 
the missing values would alter the trajectories, or when the missing data are 
not correlated with the observed data. Where along the time axis the values are 
missing may influence how much the missing data affect the 
trajectory. Dropout removes the 


:::

## Treatment: Modern Methods
...
* Mplus: can use either FIML estimation or multiple imputation
