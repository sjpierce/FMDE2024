...
MI replaces each missing value with several independent draws from the 
variable's conditional distribution, leaving us to pool the results of analyzing 
*m* complete, imputed datasets. 

* Uses all available data 
* Requires 2 models: imputation & analysis
* Pools parameter estimates across analyses of imputed datasets
* Preserves uncertainty associated with imputation
* Quantifies influence of missing data on parameter estimation
* Works well with MCAR and MAR
...
analysis reflects your hypotheses, using predictors and covariates to predict 
the outcome variables. It must be run on each imputed dataset. Pooling the 
parameter estimates across the set of analysis models gives you the final 
